    ...
    <div className="sticky top-0 z-10 mb-6 space-y-4 bg-white pb-4">
      ...
      {showFilters && (
        <div className="rounded-lg border bg-white p-4 shadow-sm">
          {CATEGORIES.WORK_TYPE.subcategories.map((category) => (
            <div key={category.id} className="mb-6 last:mb-0">
              <h3 className="mb-3 font-medium text-gray-900">{category.name}</h3>
              <div className="flex flex-wrap gap-2">
                <Button
                  size="sm"
                  variant={subcategory === category.id ? 'primary' : 'outline'}
